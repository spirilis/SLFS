...
int SLFS::open(const uint8_t *filename, int32_t mode)
{
...
int SLFS::del(const uint8_t *filename)
{
	if (filehandle)
		return false;  // In case the file we have open is the one the user is trying to delete...
